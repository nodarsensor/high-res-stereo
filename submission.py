...
parser.add_argument(
    "--clean", type=float, default=-1, help="clean up output using entropy estimation"
)
parser.add_argument(
    "--testres", type=float, default=0.5, help="test time resolution ratio 0-x"
)
parser.add_argument(
    "--max_disp", type=float, default=-1, help="maximum disparity to search for"
)
parser.add_argument(
    "--level",
    type=int,
...
def main():
    ...
    for inx in range(len(test_left_img)):
        ...
        cv2.imwrite(
            "%s/%s-ent.png" % (args.outdir, idxname.split("/")[0]),
